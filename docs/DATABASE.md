...
Columns: id, name, description, status, sort_order, created_at, updated_at, deleted_at

Generated Columns: `name_active`

Index:
- PRIMARY KEY (id)
- UNIQUE (`name_active`)

Soft Delete: ใช้ `deleted_at` (Unique Index อยู่บน Generated Column เพื่อไม่ต้องแก้ `name` เดิมตอน Soft Delete)

หมายเหตุ: คอลัมน์ `code`/`code_active` และ UNIQUE (`code_active`) ถูกลบออกถาวรโดย `013_departments_drop_code.sql` (ดู Migration History) — ระบบเลิกใช้ Department Code ทั้งระบบแล้ว
...
- `013_departments_drop_code.sql` — ลบ UNIQUE INDEX `departments_code_active_unique`, Generated Column `code_active`, และคอลัมน์ `code` ออกจากตาราง `departments` ถาวร (Executed — ยกเลิกการใช้งาน Department Code ทั้งระบบ ตรวจสอบแล้วไม่มี Foreign Key อ้างอิง) — Backup ข้อมูลก่อน Execute ไว้ที่ `database/backups/departments_backup_20260723_pre_drop_code.sql`

### Public Website Stage 2
- ไม่มี Migration ใหม่ — Reuse ตารางเดิมทั้งหมด (`news`, `legislation`, `documents`, `gallery`, `departments`, `employee`, `activities`) ผ่าน Model เดิม 100% ไม่มีการแก้ไข Schema
...
## Current Database Status (อัปเดตล่าสุด — หลัง Public Website Stage 2)

Tables: users, departments, employee, news, legislation, documents, gallery, activities, role_permissions, activity_logs (รวม 10 ตาราง — ไม่มีตารางใหม่จาก Public Website Stage 2)
